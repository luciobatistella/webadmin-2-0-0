...
<template>
  <div class="relative flex items-center gap-2" ref="root">
    ...
    <div class="relative">
      <button
        class="flex items-center gap-2 rounded-full border bg-white px-3 py-1 text-sm shadow-sm hover:bg-zinc-50 pr-1 pl-2"
        :class="open ? 'ring-2 ring-blue-500 ring-offset-1 ring-offset-white' : ''"
        @click="toggle">
      ...
      <div v-if="open" class="absolute right-0 z-50 mt-2 w-56 rounded-md border bg-white p-1 shadow-lg">
        <ul class="text-sm p-1">
          ...
          <li><hr class="my-1"/></li>
          <li>
            <button class="flex w-full items-center gap-2 rounded px-2 py-1 text-left hover:bg-zinc-50" @click="onAction('exportar')">
              <svg class="h-4 w-4 text-zinc-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M7 10l5-5 5 5"/><path d="M12 15V3"/></svg>
              exportar para planilha
            </button>
          </li>
          <li>
            <button class="flex w-full items-center gap-2 rounded px-2 py-1 text-left hover:bg-zinc-50" @click="onAction('importar')">
              <svg class="h-4 w-4 text-zinc-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><path d="M7 10l5 5 5-5"/><path d="M12 4v11"/></svg>
